...
class Sphere : public GeometricPrimitive {
  public:
	...
	bool hit(const Ray3 &ray, HitRecord &hit_record) const override {
		...
		if (t1 >= 0)
			t_min = t1;
		if (t2 >= 0 && (t_min == std::numeric_limits<T>::max() || t2 < t_min))
			t_min = t2;
...
class RightPrism : public GeometricPrimitive {
  public:
	...
	void set_radius(T new_radius) { *this = RightPrism(base_center, new_radius, height, order); }

	T get_height() const { return height; }

	void set_height(T new_height) { *this = RightPrism(base_center, radius, new_height, order); }
...
class RightPyramid : public GeometricPrimitive {
  public:
	...
	void set_radius(T new_radius) { *this = RightPyramid(base_center, new_radius, height, order); }

	T get_height() const { return height; }

	void set_height(T new_height) { *this = RightPyramid(base_center, radius, new_height, order); }
