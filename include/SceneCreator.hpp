  ...
  public:
    ...
    Scene create_scene() const override {
        ...
        */
        ...
        scene.add_object(std::make_shared<Object>(left_wall, Material(sRGB::RED, 0.6)));

        // Green Right Wall
        auto right_wall = std::make_shared<Quad>(RBN, RTN - RBN, RBF - RBN);
        scene.add_object(std::make_shared<Object>(right_wall, Material(sRGB::BLUE, 0.6)));
        ...
        scene.add_object(std::make_shared<Object>(floor, Material(sRGB::GREEN, 0.6)));
    ...
    SceneView get_view() const override {
        SceneView view;
        view.pos = {width * 0.8, height / 2, -0.1};
        view.dir = {-0.35, 0, -1};
        return view;
    }
};
