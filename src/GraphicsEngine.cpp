...
void draw_objects_ui(Scene& scene) {
    if (ImGui::CollapsingHeader("Objects")) {
        ...
        const char* primitives[] = { Sphere::type.c_str(), Plane::type.c_str(), Box::type.c_str(), RightPrism::type.c_str(), RightPyramid::type.c_str() };
        ...
        if (ImGui::Button("Add object")) {
            ...
            if (primitives[selected_primitive] == Sphere::type.c_str()) primitive = std::make_shared<Sphere>(sphere_center, sphere_radius);
            if (primitives[selected_primitive] == Plane::type.c_str()) primitive = std::make_shared<Plane>(plane_normal, plane_offset);
            if (primitives[selected_primitive] == Box::type.c_str()) primitive = std::make_shared<Box>(Vec3(box_center) - Vec3(box_sizes) / 2.f, Vec3(box_center) + Vec3(box_sizes) / 2.f);
            if (primitives[selected_primitive] == RightPrism::type.c_str()) primitive = std::make_shared<RightPrism>(prism_base_center, prism_radius, prism_height, prism_order);
            if (primitives[selected_primitive] == RightPyramid::type.c_str()) primitive = std::make_shared<RightPyramid>(pyramid_base_center, pyramid_radius, pyramid_height, pyramid_order);

            std::shared_ptr<Object> object;
            Color color(float_rgb);
            std::shared_ptr<Material> material;
            if (materials[selected_material] == MATTE_PLASTIC) object = std::make_shared<Object>(primitive, MattePlastic(color));
            if (materials[selected_material] == GLOSSY_PLASTIC) object = std::make_shared<Object>(primitive, GlossyPlastic(color));
            if (materials[selected_material] == METAL) object = std::make_shared<Object>(primitive, Metal(color));
            if (materials[selected_material] == GLASS) object = std::make_shared<Object>(primitive, Glass(color));
...
bool process_key_input(Camera& camera) {
    ...
    // Movement
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::W)) { camera.pos += camera.dir * Settings::CAMERA_MOVEMENT_SPEED; is_key_pressed = true; }
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::A)) { camera.pos -= camera.right * Settings::CAMERA_MOVEMENT_SPEED; is_key_pressed = true; }
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::S)) { camera.pos -= camera.dir * Settings::CAMERA_MOVEMENT_SPEED; is_key_pressed = true; }
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::D)) { camera.pos += camera.right * Settings::CAMERA_MOVEMENT_SPEED; is_key_pressed = true; }
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::PageUp)) { camera.pos -= camera.up * Settings::CAMERA_MOVEMENT_SPEED; is_key_pressed = true; }
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::PageDown)) { camera.pos += camera.up * Settings::CAMERA_MOVEMENT_SPEED; is_key_pressed = true; }

    // Rotation
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::Up) && camera.get_zenith() <= Settings::MAX_ZENITH_RADIANS) { camera.rotate_vertically(-Settings::CAMERA_ROTATION_SPEED); is_key_pressed = true; }
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::Down) && camera.get_zenith() >= -Settings::MAX_ZENITH_RADIANS) { camera.rotate_vertically(Settings::CAMERA_ROTATION_SPEED); is_key_pressed = true; }
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::Left)) { camera.rotate_horizontally(Settings::CAMERA_ROTATION_SPEED); is_key_pressed = true; }
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::Right)) { camera.rotate_horizontally(-Settings::CAMERA_ROTATION_SPEED); is_key_pressed = true; }
