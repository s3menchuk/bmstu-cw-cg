...
constexpr float FOV =
    std::numbers::pi / 2.0f;  // std::numbers::pi * 2.0f / 3.0f;
constexpr float NEAR = 1.0f;
constexpr float FAR = 1000.0f;

constexpr float MAX_ZENITH_DEGREES = 75.0f;
constexpr float MAX_ZENITH_RADIANS =
    std::numbers::pi / 2 * MAX_ZENITH_DEGREES / 90;
...
void draw_camera_ui(Camera &camera) {
  if (ImGui::CollapsingHeader("Camera")) {
    ...
    if (ImGui::SliderFloat("zenith", &zenith, -Settings::MAX_ZENITH_DEGREES,
                           Settings::MAX_ZENITH_DEGREES))
...
void draw_objects_ui(Scene &scene) {
  if (ImGui::CollapsingHeader("Objects")) {
    ...
    for (size_t i = 0; i < scene.objects.size();) {
      ...
      if (counter.find(obj_type) == counter.end()) counter[obj_type] = 0;
      counter[obj_type]++;
      const std::string name =
          obj_type + "##" + std::to_string(counter[obj_type]);
      if (ImGui::TreeNode(name.c_str())) {
        if (obj_type == Sphere::type) {
          std::shared_ptr<Sphere> sphere(
              std::dynamic_pointer_cast<Sphere>(obj->operator->()));

          ImGui::Text("Center");
          ImGui::SameLine();
          std::array<float, 3> center(sphere->center);
          if (ImGui::DragFloat3(("##Center" + std::to_string(i)).c_str(),
                                center.data()))
          ...
          if (ImGui::DragFloat(("##Radius" + std::to_string(i)).c_str(),
                               &radius, 1.0f, 0.01, 100))
            sphere->set_radius(radius);
        } else if (obj_type == Plane::type) {
          std::shared_ptr<Plane> plane(
              std::dynamic_pointer_cast<Plane>(obj->operator->()));

          ImGui::Text("Normal");
          ImGui::SameLine();
          std::array<float, 3> normal(plane->get_normal());
          if (ImGui::DragFloat3(("##Normal" + std::to_string(i)).c_str(),
                                normal.data()))
            plane->set_normal(normal);  // TODO

          ImGui::Text("Offset");
          ImGui::SameLine();
          ImGui::DragFloat(("##Offset" + std::to_string(i)).c_str(),
                           &plane->offset, 1.0f, 0.01, 100);
        } else if (obj_type == Box::type) {
          std::shared_ptr<Box> box(
              std::dynamic_pointer_cast<Box>(obj->operator->()));
          ...
          std::shared_ptr<RightPrism> prism(
              std::dynamic_pointer_cast<RightPrism>(obj->operator->()));
          ...
          if (ImGui::DragFloat(("##Height" + std::to_string(i)).c_str(),
                               &height, 1.0f, 0.01, 100))
            prism->set_height(height);

          ImGui::Text("Order");
          ImGui::SameLine();
          int order = prism->get_order();
          if (ImGui::DragInt(("##Order" + std::to_string(i)).c_str(), &order,
                             1.0f, 3, 16))
            prism->set_order(order);
        } else if (obj_type == RightPyramid::type) {
          std::shared_ptr<RightPyramid> pyramid(
              std::dynamic_pointer_cast<RightPyramid>(obj->operator->()));
    ...
    const char *primitives[] = {Sphere::type.c_str(), Plane::type.c_str(),
                                Box::type.c_str(), RightPrism::type.c_str(),
                                RightPyramid::type.c_str()};
    static int selected_primitive = 0;

    if (ImGui::BeginCombo("##combo_primitives",
                          primitives[selected_primitive])) {
    ...
    if (ImGui::Button("Add object")) {
      ...
      if (primitives[selected_primitive] == Box::type.c_str())
        primitive =
            std::make_shared<Box>(Vec3(box_center) - Vec3(box_sizes) / 2.f,
                                  Vec3(box_center) + Vec3(box_sizes) / 2.f);
      if (primitives[selected_primitive] == RightPrism::type.c_str())
        primitive = std::make_shared<RightPrism>(
            prism_base_center, prism_radius, prism_height, prism_order);
      if (primitives[selected_primitive] == RightPyramid::type.c_str())
        primitive = std::make_shared<RightPyramid>(
            pyramid_base_center, pyramid_radius, pyramid_height, pyramid_order);
...
bool process_key_input(Camera &camera) {
  ...
  if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::Up) &&
      camera.get_zenith() <= Settings::MAX_ZENITH_RADIANS) {
...
int main() {
  sf::RenderWindow window(sf::VideoMode({Settings::WIDTH, Settings::HEIGHT}),
                          "Graphics Engine");
  window.setFramerateLimit(60);
  if (!ImGui::SFML::Init(window)) return -1;

  auto canvas = std::make_unique<SFML_Canvas>(
      Settings::WIDTH, Settings::HEIGHT);  // std::unique_ptr<Canvas>
  Camera camera(Settings::INIT_CAMERA_POS, Settings::INIT_CAMERA_DIR,
                Settings::FOV, Settings::ASPECT, Settings::NEAR, Settings::FAR);

  Scene scene;
  scene.ambient_color = WHITE;
  scene.ambient_intensity = 0.2;

  scene.add_object(std::make_shared<Object>(
      std::make_shared<Box>(Vec3(0, -0.5, -0.5), Vec3(-1, -1.5, -1.5)),
      GlossyPlastic(RED)));
  scene.add_object(std::make_shared<Object>(
      std::make_shared<Sphere>(Vec3(-5, 10, 0), 5), Metal(BLUE), false));
  scene.add_object(std::make_shared<Object>(
      std::make_shared<Plane>(Vec3(-1, 0, 0), 0), GlossyPlastic(GRAY)));
