...
TODO:
                - GUI | Ok
                - Добавить реализацию оставшихся
объектов | Ok
                - Reorganization object class
hierarchy (Object, GeometricPrimitive,
...) | Ok???
                - Object rotation
                - Emission materials
                ...
                - Antialiasing (several rays per
pixel)
                - Real diffuse (cancel - too much
expensive)
  ...
  if (ImGui::CollapsingHeader("Objects")) {
    ...
    for (size_t i = 0; i < scene.objects.size();) {
      ...
      if (ImGui::TreeNode(name.c_str())) {
        ...
        ImGui::Text("Color");
        ImGui::SameLine();
        std::array<float, 3> float_rgb = obj->material.diffuse.as_hdr_array();
...
int main() {
  ...
  scene.ambient_color = Color::WHITE;
  scene.ambient_intensity = 1;

  scene.add_object(std::make_shared<Object>(std::make_shared<Box>(Vec3(0, -0.5, -0.5), Vec3(-1, -1.5, -1.5)), GlossyPlastic(Color::RED)));
  scene.add_object(std::make_shared<Object>(std::make_shared<Sphere>(Vec3(-5, 10, 0), 5), Metal(Color::BLUE), false));
  scene.add_object(std::make_shared<Object>(std::make_shared<Plane>(Vec3(-1, 0, 0), 0), GlossyPlastic(Color::GRAY)));
  scene.add_object(std::make_shared<Object>(std::make_shared<Sphere>(Vec3(-0.5, 0.5, -0.75), 0.5), Metal(Color::GREEN)));
  scene.add_object(std::make_shared<Object>(std::make_shared<RightPrism>(Vec3(-8, 0, 0), 5, 6, 16), Glass(Color::GREEN), false));
  scene.add_object(std::make_shared<Object>(std::make_shared<RightPyramid>(Vec3(0, 10, 8), 5, 8, 3), MattePlastic(Color::ORANGE), false));
  ...
  scene.add_light(std::make_shared<DirectionLight>(Vec3(1, -1, -1), Color::WHITE, 1));
