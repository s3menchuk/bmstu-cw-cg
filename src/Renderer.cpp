...
Color RayTracingRenderer::background_color(const Scene &scene, const Ray3 &ray) const {
  ...
  return 5 * scene.background_color * (1.0f - k) + Color::WHITE * k;
}
...
Color RayTracingRenderer::trace_ray(const Scene &scene, const Ray3 &ray, size_t depth) {
  ...
  //     light->get_intensity(hit_record.point); specular += closest_obj->material.specular * light->color *
  //                 std::pow(std::max(hit_record.normal.dot(H), 0.0f), closest_obj->material.shininess) * light->get_intensity(hit_record.point);
  //   }
  // }
