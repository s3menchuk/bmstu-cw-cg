...
void sort_vertices_ccw3d(std::vector<Vec3>& vertices) {
	...
	// Нормаль к плоскости через два первых вектора
	Vec3 normal = v1.cross(v2);
	normal.normalize();

	// Создаём базис в плоскости: u и v — две ортонормальные оси
	Vec3 u = v1;               // Первая ось вдоль v1
	u.normalize();
	Vec3 v = normal.cross(u);  // Вторая ось ортогональна u и нормали
	v.normalize();

	// Сортировка по углу atan2 вдоль u и v
